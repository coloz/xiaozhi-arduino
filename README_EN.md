...
- `ClientConfig` can limit JSON and Opus payload sizes; the defaults are 8192 and 4096 bytes, respectively. Transport's binary limit automatically includes the selected protocol's wire header.
...
The built-in adapter switches ArduinoWebsockets 0.5.x to fragment notifications and reassembles text and binary messages against the exact limits installed by Client; it closes immediately on a cumulative oversize or invalid fragment sequence. Its own capacity ceiling is 16384 bytes. In ArduinoWebsockets 0.5.4, changing the policy does not rebuild the existing StreamBuilder, so the first fragmented message of each new client still briefly has a second dependency-owned copy; an individual frame is also allocated before the callback. For an untrusted endpoint, build ArduinoWebsockets with `_WS_CONFIG_MAX_MESSAGE_SIZE` no smaller than the largest required runtime wire frame and no larger than 16384. The complete examples use `compiler.cpp.extra_flags=-D_WS_CONFIG_MAX_MESSAGE_SIZE=16384`. When that compile-time value is also visible while building the adapter, `setLimits()` rejects any larger runtime request; otherwise the adapter still applies the runtime guard after the dependency callback. A custom Transport can enforce the limit even earlier while reading.
